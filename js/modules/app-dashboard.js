    ...
    container.innerHTML = mapaData.map(area => {
        const posicionesHtml = area.posiciones.map(pos => {
            ...
            if (maquinaSupervisora?.procesoId) {
                procesosActivos.push({ nombre: maquinaSupervisora.procesoNombre || 'Proceso', estado: maquinaSupervisora.estado || 'asignado', piezas: maquinaSupervisora.piezasCompletadas || 0 });
            }
            if (maquinaSupervisora?.colaProcesos && Array.isArray(maquinaSupervisora.colaProcesos)) {
                maquinaSupervisora.colaProcesos.forEach(proc => { procesosActivos.push({ nombre: proc.procesoNombre || 'En cola', estado: 'pendiente', piezas: 0 }); });
            }
            if (estadoMaquina?.procesosSimultaneos && Array.isArray(estadoMaquina.procesosSimultaneos)) {
                estadoMaquina.procesosSimultaneos.forEach(proc => {
                    if (!procesosActivos.some(p => p.nombre === proc.procesoNombre)) {
                        procesosActivos.push({ nombre: proc.procesoNombre || 'Proceso', estado: 'en_proceso', piezas: proc.piezasCompletadas || 0 });
                    }
                });
            }
            if (asignacion?.procesoNombre && !procesosActivos.some(p => p.nombre === asignacion.procesoNombre)) {
                procesosActivos.push({ nombre: asignacion.procesoNombre, estado: asignacion.estado || 'asignado', piezas: asignacion.piezasCompletadas || 0 });
            }
            if (asignacion?.colaProcesos && Array.isArray(asignacion.colaProcesos)) {
                asignacion.colaProcesos.forEach(proc => {
                    if (!procesosActivos.some(p => p.nombre === proc.procesoNombre)) {
                        procesosActivos.push({ nombre: proc.procesoNombre || 'En cola', estado: 'pendiente', piezas: 0 });
                    }
                });
            }
            ...
            let colorBorde = '';
            if (tieneAsignacion) {
                if (estaTrabajando || piezasHoy > 0) { estado = 'trabajando'; colorBorde = '#10b981'; }
                else { estado = 'asignado'; colorBorde = '#f59e0b'; }
            } else if (operadorNombre) { estado = 'disponible'; colorBorde = '#6b7280'; }

            let procesoTexto = '';
            let procesoTooltip = '';
            if (totalProcesos > 1) { procesoTexto = `${totalProcesos} procesos`; procesoTooltip = procesosActivos.map(p => p.nombre).join(', '); }
            else if (totalProcesos === 1) { procesoTexto = procesosActivos[0].nombre; procesoTooltip = procesoTexto; }
            else { procesoTexto = asignacion?.procesoNombre || estadoMaquina?.procesoNombre || ''; procesoTooltip = procesoTexto; }

            const tooltip = tieneAsignacion
                ? `${S(operadorNombre || 'Operador')} - ${S(procesoTooltip)}${piezasHoy > 0 ? ` (${piezasHoy} pzas)` : ''}`
                : operadorNombre ? `${S(operadorNombre)} - Sin proceso` : S(pos.nombre || pos.id);

            if (!operadorNombre && !tieneAsignacion) {
                return `<div class="workstation empty clickable" data-tooltip="${S(pos.nombre || pos.id)}" data-id="${pos.id}" onclick="showPosicionDetalle('${pos.id}')"><span class="workstation-code">${pos.id}</span></div>`;
            }

            const iniciales = operadorNombre ? operadorNombre.split(' ').map(n => n[0]).join('').substring(0, 2).toUpperCase() : '??';
